...
spec = do
  ...
  describe "Examples" $ do
    ...
    it "Function Call and Function Def" $ do
      parseAssembler (T.concat [functionCallEAS,functionDefinitionEAS]) `shouldBe` Right (functionCallIL ++ functionDefinitionIL)
  ...
  describe "Files" $ do
    ...
    it "hello2"  $ do hello2Either  `shouldBe` Right (hello2IL ++ [D "writestr"])
    ...
    it "fact"    $ do factEither    `shouldBe` Right factIL
    it "bottles" $ do bottlesEither `shouldBe` Right bottlesIL


--parseFromFile :: String
parseFromFile name = fmap parseAssembler $ runIO $ T.readFile $ "src/test/resources/eas/" ++ name ++ ".eas"
