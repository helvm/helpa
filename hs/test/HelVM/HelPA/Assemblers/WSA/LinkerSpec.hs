...
spec = do
  ...
  describe "linkLibFile" $ do
    forM_ [ "io"
      ...
      it fileName $ do
        linkLibFile fileName `goldenShouldSafeExceptT` buildAbsolutePathToIlFile ("linkLibFile" </> fileName)
  ...
  describe "linkAppFile" $ do
    describe "original" $ do
      forM_ [ "prim"
        ...
        it fileName $ do
          linkAppFile fileName `goldenShouldSafeExceptT` buildAbsolutePathToIlFile ("linkAppFile" </> "original" </> fileName)
 ...
 --           , "euclid"
        ...
        it fileName $ do
          linkAppFile1 fileName `goldenShouldSafeExceptT` buildAbsolutePathToIlFile ("linkAppFile" </> "from-eas" </> fileName)
